...
const abi = require("../config/abiTUSD");

let TUSDCONTRACTADDRESS = process.env.TUSDCONTRACTADDRESS;
...
const contract = new web3.eth.Contract(abi, TUSDCONTRACTADDRESS, error => {
  console.log(error);
});
          ...
          if (events != undefined || events != null) {
            if (events.length > 0) {
              events.forEach(async event => {
                wallets.findOne(
                  { "ETH.address": event.returnValues.to },
    ...
    web3.eth.getTransactionReceipt(event.transactionHash, function(
      ...
      if (err || result == null) {
        ...
        let record = {
          ...
          value: Number(event.returnValues.value / Math.pow(10, 18)).toFixed(8),
          coin: "TUSD",
          userId: userId,
