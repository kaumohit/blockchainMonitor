    ...
    (error, result, body) => {
      if (body != undefined) {
        ...
        for (let i = 0; i < output.length; i++) {
          console.log("output address ", output[i].address_hash);
          console.log("Asset : ", output[i].asset);
            ...
            wallets.findOne(
              ...
              (error, searchResult) => {
                if (error) {
                  ...
                  updateTransaction(body, output[i], "searchResult.userId");
                }
              }
            );
          }
...
function updateTransaction(txn, output, user_id) {
  try {
    let record = {
      txHash: txn.txid,
