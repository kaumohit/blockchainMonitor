...
const abi = require("../config/abiQTUM");

let QTUMCONTRACTADDRESS = process.env.QTUMCONTRACTADDRESS;
...
const contract = new web3.eth.Contract(abi, QTUMCONTRACTADDRESS, error => {
  console.log(error);
});
    ...
    web3.eth.getBlockNumber(async (error, toBlock) => {
      if (!error) {
        if (toBlock > fromBlock) {
          console.log(toBlock)
          await getPastEvents(toBlock, toBlock);
          fromBlock = toBlock;
        } else {
    ...
    web3.eth.getTransactionReceipt(event.transactionHash, function(
      ...
      if (err || result == null) {
        ...
        let record = {
          ...
          value: Number(event.returnValues._value / Math.pow(10, 18)).toFixed(8),
          coin: "QTUM",
          userId: userId,
